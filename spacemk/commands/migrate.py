...
def migrate(ctx):
    ...
    code_publisher = CodePublisher(config=config.get("code_publisher", {}), console=ctx.obj["console"])
    code_publisher.publish()

    # Create the Spacelift manager stack and trigger a run to create the entities
    importer = Importer(config=config.get("importer", {}), console=ctx.obj["console"])
    importer.create()

    # Upload Terraform state files to the state backend
    state_uploader = StateUploader(config=config.get("state_uploader", {}), console=ctx.obj["console"])
